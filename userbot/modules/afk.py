...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"VeronicaAssistant\n**• Veronica ᴛᴇʟᴀʜ ᴀꜰᴋ**\
        \n**• ᴀʟᴀꜱᴀɴ :** `{string}`")
    else:
        await afk_e.edit("VeronicaAssistant\n**• Veronica ᴛᴇʟᴀʜ ᴀꜰᴋ**")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**Veronica ᴛᴇʟᴀʜ ᴋᴇᴍʙᴀʟɪ !!**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"Veronica Assistant\n`{ALIVE_NAME}` **ᴀꜰᴋ** {afk_since} **ʏᴀɴɢ ʟᴀʟᴜ.**\
                        \n**• ᴀʟᴀꜱᴀɴ :** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"Veronica Assistant\n**Veronica ᴍᴀꜱɪʜ ᴀꜰᴋ** {afk_since} **ʏᴀɴɢ ʟᴀʟᴜ.**\
                            \n**• ᴀʟᴀꜱᴀɴ :** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
